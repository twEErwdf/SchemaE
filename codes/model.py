...
class KGEModel(nn.Module):
    def __init__(self, args, model_name, nentity, nrelation, hidden_dim, gamma, train_g_bidir,
        ...
        # )
        self.relation_embedding = nn.Parameter(torch.ones(nrelation, self.relation_dim))
    ...
    def train_step(model, type_model, ent_type_feat, train_g_bidir, optimizer, train_iterator, args):
        ...
        negative_score, _ = model((positive_sample, negative_sample), ent_type_feat_emb, mode=mode)
        ...
        # positive_score(1024)
        positive_score, head = model(positive_sample, ent_type_feat_emb)
        ...
        if args.regularization != 0.0:
            ...
            loss = loss + regularization + 0.3 * type_loss
    ...
    def test_step(model, type_model, ent_type_feat, true_and_false_edges, test_triples, all_true_triples, args):
        ...
        if args.countries:
            ...
            with torch.no_grad():
                ent_type_feat_emb = type_model.get_type_feat_emb(ent_type_feat)
                y_score, _ = model(samples, ent_type_feat_emb)
