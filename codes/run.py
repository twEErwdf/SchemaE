...
def parse_args(args=None):
    ...
    parser.add_argument('--cuda', default='1', action='store_true', help='use GPU')
    ...
    parser.add_argument('--data_path', type=str, default='../datatype/JF17k')
...
def main(args):
    ...
    valid_triples = read_triple_test(os.path.join(args.data_path, 'valid.txt'))
    logging.info('#valid: %d' % len(valid_triples))
    test_triples = read_triple_test(os.path.join(args.data_path, 'test.txt'))
    ...
    if args.do_train:
        ...
        steps_per_epoch = (len(train_triples) + args.batch_size -1)// args.batch_size
        ...
        for step in range(init_step, args.max_steps):
            ...
            if (step + 1) % steps_per_epoch == 0:
                ...
                logging.info('Epoch %d finished in %f seconds' % ((step + 1) // steps_per_epoch, epoch_duration))

        # 打印显存使用情况(训练结束时)
        print_memory_usage()
        total_training_time = time.time() - start_time  # 计算总的训练时间
        logging.info('Total training time: %f seconds' % total_training_time)
